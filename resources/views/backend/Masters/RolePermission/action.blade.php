            ...
            <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-xl">
                    <div class="modal-content">
                        <form enctype="multipart/form-data" name="actionform" id="actionform">
                            <input type="hidden" id="saveurl" value="{{ url('action/saveData') }}" />
                            <input type="hidden" id="recordid" name="recordid" value="" />
                            <input type="hidden" id="mode" name="mode">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5 text-dark" id="exampleModalLabel">Manage Actions</h1>
                            ...
                            <div class="modal-body" style="color:black;font-weight:600">
                                ...
                                <div class="row pb-3">
                                    <div class="col-md-6">
                                        <label for="block" class="form-label">Action Name<span style="color:red" title="Mandatory">*</span></label>
                                        <input type="text" class="form-control" placeholder="Enter action Name" id="actionname" name="actionname">
                                    </div>
                                    <div class="col-md-6">
                                            ...
                                            <tr>
                                                ...
                                                <!-- <td>
                                                    <div class="btn-group">
                                                        ...
                                                        <a href='#' class='editbtn'  onclick='showEdit({{ $act->id }})'
                                                            title='Edit'><img src='assets/previous/user.svg'
                                                                style='height:20px; width:20px' /></a>&nbsp&nbsp
                                                        <a href='javascript:void(0)'
                                                            onclick="deleteData('{{ url('action/deleteData') }}/{{ $act->id }}')"
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status) {
                        ...
                        if ($("#mode").val() === 'add') {
                            $("#actionform")[0].reset(); // Reset the form
                        } else {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.action) {
                ...
                myModal.show();
                document.getElementById("actionname").value = data.action.action_name;

            } else {
                console.error('Action data not found');
            }
        },
